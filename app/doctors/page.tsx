...
import { Edit2, Trash2, Plus, MonitorCheck } from 'lucide-react';

export default function DoctorsPage() {
  const { doctors, updateDoctor, deleteDoctor, patients } = useData();
  ...
  const columns = [
    {
      ...
      render: (d: Doctor) => {
        const waitingCount = patients.filter(
          (p) => p.doctorId === d.id && (p.status === 'Pending' || p.consultationStatus)
        ).length;
        return (
          <button
            onClick={() => router.push('/doctor-consultation')}
            className="text-teal-600 dark:text-teal-400 hover:underline font-medium"
            title={`View queue (${waitingCount} patients)`}
          >
            {d.name}
            {waitingCount > 0 && (
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <p className="text-gray-700 dark:text-gray-200 mb-4">Are you sure you want to delete this doctor? All associated patient assignments will be cleared.</p>
        <div className="flex gap-2">
          <button
